...
def privacy_test(
    representation: Literal["ECFP4", "ECFP6", "MACCS", "graph", "rdkit", "transformer_vector", "transformer_matrix", "custom"],
    result_folder: str,
    ...
    # Run moreno 
    momain(task="train", dataset=dataset, split=split, representations=[representation], hyperparameter_optimization_time=hyperparameter_optimization_time, result_folder=result_folder, save_csv_in_results=True, dataset_path=dataset_path, custom_representation_function=custom_representation_function) 
    # Run LeakPro
    if dataset == "herg":
        attack_data_fraction = 0.2
    lemain(representation=representation, result_folder=result_folder, attack_data_fraction=attack_data_fraction)
...
def main():
    ...
    parser.add_argument(
        '--representation',
        type=str,
        choices=["ECFP4", "ECFP6", "MACCS", "graph", "rdkit", "transformer_vector", "transformer_matrix", "custom"],
        help="Representation type."
    )
    parser.add_argument('--result_folder', type=str, help="Path to the folder where the result will be stored.")
    parser.add_argument(
        '--dataset',
        type=str,
