...
from moreno.data_modules.datasets_and_collate_functions import GraphDataset, mol_collate_fn, CNNDataset, CNN_collate_fn
import pandas as pd
import torch
...
class TestPredictor:
    ...
    def __init__(self, representation: str, model: MLPLightning | MPNNLightning, datafile: Path) -> None:
    ...
    def get_y_pred_and_y_true(self) -> Dict[str, NDArray[np.float64]]:
        ...
        if self.representation == "graph":
            ...
            assert self.input_vec_dim == self.model.input_vec_dim, "Input vector dimension of model does not line up with the model representation."
...
class BaselinePredictor:
    ...
    def get_y_pred_and_y_true(self):
        ...
        y_pred = np.random.choice([0, 1], size=len(test_data), p=[1 - class_imbalance, class_imbalance])
