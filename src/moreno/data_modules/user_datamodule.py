...
class UserDataModule(CustomDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        if not (
            ...
            if len(self.dataset_paths) == 1:
                print("Found one dataset. This will be split into train val and test according to the split")
                data = pd.read_csv(self.dataset_paths[0])
                self.sanity_check(data)
                data = data.sample(frac=1).reset_index(drop=True)
                self.train_data = data.iloc[: int(len(data) * self.split[0])]
                self.validation_data = data.iloc[int(len(data) * self.split[0]) : int(len(data) * (self.split[0] + self.split[1]))]
                self.test_data = data.iloc[int(len(data) * (self.split[0] + self.split[1])) : ]
            ...
            #     self.validation_data = validation_data
                ...
                print(f"Found 3 datasets. Using the first one ({self.dataset_paths[0]}) as training data, the second one ({self.dataset_paths[1]}) as validation data, and the last one ({self.dataset_paths[2]}) as test data.")
                train_data = pd.read_csv(self.dataset_paths[0])
                self.sanity_check(train_data)
                self.train_data = train_data
                validation_data = pd.read_csv(self.dataset_paths[1])
                ...
                raise ValueError(f"Provided {len(self.dataset_paths)} dataset paths. Expected 1 or 3.")
    ...
    def setup(self, stage: str) -> None:
        if stage == "fit":
            ...
            Config.set_pos_weights(torch.tensor([total_negative_samples/total_positive_samples]))
    ...
    def sanity_check(self, data: pd.DataFrame) -> None:
        ...
        if set(data.columns) != expected_columns:
            raise ValueError(f"Column names of provided data are not as expected. Expected {expected_columns}, but got {set(data.columns)}.")
        if data.columns.value_counts().to_dict() != {col: 1 for col in expected_columns}:
            raise ValueError(f"Provided data had multiple columns for smiles or label. Expected exactly one of each {expected_columns}, but got {data.columns.tolist()}.")
        if len(data) < 10:
            raise ValueError(f"Found less than 10 molecules in the provided data. That's way too few. Amount is {len(data)}.")
        if not data["label"].isin([0, 1]).all():
            raise ValueError(f"The 'label' column contains values other than 0 and 1. This package only supports binary classification. Unique values found: {data['label'].unique()}.")
        ...
        if invalid_smiles.any():
            ...
            raise ValueError(f"The 'smiles' column contains these invalid SMILES strings: {invalid_entries.tolist()}")