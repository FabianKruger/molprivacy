...
def subset_mol_sizes_from_population(population: List[str], subset: List[str]) -> Tuple[List[int], List[int]]:
    subset_sizes = []
    complement_sizes = []
...
def create_dataframe(representations: List[str], dataset: str, attack: str, base_path: Path, temp_folder: Path) -> pd.DataFrame:
    data = {
        "Representation": [],
        "Set": [],
        "Size": []
    ...
    for rep in representations:
        ...
        for job in range(1, 21):
            try:
                ...
                leak_df = load_data(base_path / dataset / f"job_{job}" / "..." / "privacy" / "results" / "true_positives_at_FPR0" / f"{attack}.csv")
        ...
        for pop, leaked in zip(population_data, leakage_data):
            ...
            data["Set"].extend(["Subset"] * len(sub_sizes) + ["Population"] * len(pop_sizes))
...
def plot_dataframe(dfs: List[pd.DataFrame], figure_folder: Path, attacks: List[str], datasets: List[str]) -> None:
    ...
    for row, dataset in enumerate(datasets):
        for col, attack in enumerate(attacks):
            ...
            df['Set'] = df['Set'].replace(set_labels)  # Update the labels in the dataframe

            sns_plot = sns.boxplot(data=df, x="Representation", y="Size", hue="Set", ax=ax, showfliers=False)
            
            unique_reps = df["Representation"].unique()
            for i, rep in enumerate(unique_reps):
                subset_sizes = df[(df["Representation"] == rep) & (df["Set"] == "Identified molecules")]["Size"]
                population_sizes = df[(df["Representation"] == rep) & (df["Set"] == "Training data")]["Size"]
                ...
                if stars != ' ':
                    ...
                    ax.text(i, star_y, stars, ha='center', va='top', color='red', fontsize=16)
...
if __name__=="__main__":
    ...
    figure_folder = Path("...")
    temp_folder = Path("...") # to save sizes in case of crashing (script takes some time to run)
